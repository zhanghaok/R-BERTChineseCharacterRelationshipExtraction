...
def train(hparams):
    ...
    # 训练集
    train_dataset = SentenceREDataset(train_file,tagset_file,pretrained_model_path,max_len)
    ...
    for epoch in range(epoch_offset,epochs):
        ...
        for i_batch,sample_batched in enumerate(tqdm(train_loader,desc="Training")):
            ...
            tag_ids = sample_batched['tag_id'].to(device)#tag_id: tensor([1, 2, 2, 2, 2, 0, 6, 7], device='cuda:0')

            model.zero_grad()
            #
            logits = model(token_ids,token_type_ids,attention_mask,e1_mask,e2_mask)#torch.Size([8, 12])
            loss = criterion(logits,tag_ids)#logits的形状：#torch.Size([8, 12]) tag_ids的形状8
            loss.backward()
            optimizer.step()
            ...
            if i_batch % 20 == 0:
                ...
                logger.info(f'raining/training loss:{running_loss / 20}--{epoch * len(train_loader) + i_batch}')
                running_loss = 0.0
        if validation_file:
            ...
            with torch.no_grad():
                ...
                logger.info(metrics.classification_report(tags_true, tags_pred, labels=list(idx2tag.keys()), target_names=list(idx2tag.values())))
                ...
                logger.info(f'Validation/f1:{f1}---epoch:{epoch}')
                logger.info(f'Validation/precision:{precision}---epoch:{epoch}')
                logger.info(f'Validation/recall:{recall}---epoch:{epoch}')
                logger.info(f'Validation/accuracy:{accuracy}---epoch:{epoch}')
